...
const AppSidebar = () => {
  ...
  return <Sidebar side="left" variant="sidebar">
      <SidebarHeader className="p-4">
        <Link to="/" className="flex items-center gap-2">
          <img alt="Logo du lycée" className="h-8 w-auto" src="/lovable-uploads/8a46a967-ca93-4dd2-a05b-de81cea80ac0.jpg" />
      ...
      <SidebarContent>
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {mainMenuItems.map(item => <SidebarMenuItem key={item.path}>
        ...
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {referentielItems.map(item => <SidebarMenuItem key={`${item.path}-${item.text}`}>
                  <SidebarMenuButton isActive={isActive(item.path)} size="default" tooltip={item.text} asChild>
                    <Link to={item.path}>
                      {item.icon}
                      <span>{item.text}</span>
